function transLaplaceTotal(a,b,ciy,cix,xi,t0)
...
mensaje('APLICAMOS TRANSFORMADA DE LAPLACE y substituimos condiciones iniciales')
...
y(t)=ilaplace(edd);
pretty(y(t))
mensaje('Y la respuesta total es')
yt(t)=y+xi
...
subplot(2,2,3)
hold on 
title('Respuesta a estado cero del sistema','FontWeight','bold','FontSize',14)
xlabel('tiempo','FontWeight','bold','FontSize',14)
fplot(y,[0,t0],'c','LineWidth',2)
grid on

subplot(2,2,4)
hold on 
title('Respuesta total','FontWeight','bold','FontSize',14)
xlabel('tiempo','FontWeight','bold','FontSize',14)
fplot(yt,[0,t0],'m','LineWidth',2)
grid on
end
